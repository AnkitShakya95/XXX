...
# Define the owner and authorized users
OWNER_ID = 7576291172  # Replace with the bot owner's Telegram user ID
AUTHORIZED_USERS = [111222333, 444555666]  # Replace with authorized user IDs

# Filter for owner commands
def owner_filter(_, __, message: Message):
    return message.from_user.id == OWNER_ID

# Filter for authorized users (including the owner)
def auth_user_filter(_, __, message: Message):
    return message.from_user.id in AUTHORIZED_USERS or message.from_user.id == OWNER_ID

# Add custom filters
owner_only = filters.create(owner_filter)
auth_user_only = filters.create(auth_user_filter)

# Commands for owner only
@bot.on_message(filters.command("owner_cmd") & owner_only)
...
@bot.on_message(filters.command("auth_cmd") & auth_user_only)
async def auth_command(client, message: Message):
    await message.reply_text("This command is for authorized users!")
    ...
    try:
        for i in range(arg-1, len(links)):
            ...
            if "visionias" in url:
             ...
             vid_id =  url.split("/")[-2]
             url =  f"https://madxpw-api-d0cbf6ac738c.herokuapp.com/{vid_id}/master.m3u8?token={raw_text4}"
